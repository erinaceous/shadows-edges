...
DISTANCE = 4  # minimum pixel distance between contour points
LENGTH = 20   # default length of normals
...
def process_image(image_path, ground_truth_path, args, header=False):
    ...
    for x, scale in enumerate([100]):  # disabling pyramid stuff for now
        s = float(scale) / 100.0
        if s < 1.0:
...
def main():
    ...
    for listval in [inputs, ground_truths]:
        for input in listval:
            for ext in ['.txt', '.list']:
                if input.endswith(ext):
                    new_inputs = [x.strip() for x in open(input).readlines()
                                  if x.strip() != '']
                    listval.extend(new_inputs)
                    listval.remove(input)
                    break

    inputs = {image_id(path): path for path in sorted(inputs)}
    ground_truths = {image_id(path): path
                     for path in sorted(ground_truths)}
    comparable_images = {}
    for image in inputs:
